...
from custom_object_detection_ieee.src.config import GUI, NUM_CLASSES, DEVICE, CLASSES, COLORS, OUT_DIR
...
while cap.isOpened():
    ...
    if ret:
        ...
        left_object = [frame_width, frame_height, frame_width, frame_height]
        ...
        if len(outputs[0]["boxes"]) != 0:
            ...
            for j, box in enumerate(draw_boxes):
                ...
                cv2.putText(
                    frame,
                    f"({xmin}, {ymin}), ({xmax}, {ymax}) {center[0]} {center[1]}",
                    (xmin, ymax + 25),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.8,
                    color[::-1],
                ...
                if xmin < left_object[0]:
                    left_object = [xmin, ymin, xmax, ymax]
        cv2.putText(
            frame,
            f"{fps:.0f} FPS",
            (15, 25),
        ...
        if len(centers) > 0:
            ...
            cv2.putText(
                frame,
                f"Average Center: ({avg_center[0]:.0f}, {avg_center[1]:.0f})",
                (0, frame_height - 25),
                cv2.FONT_HERSHEY_SIMPLEX,
                1,
                (0, 255, 0),
        ...
        if left_object[2] < frame_width / 2:
            cv2.putText(
                frame,
                f"Move Left ({left_object[2]} < {frame_width / 2})",
                (0, frame_height - 50),
                cv2.FONT_HERSHEY_SIMPLEX,
                1,
                (0, 0, 255),
        ...
        elif left_object[0] > frame_width / 2:
            cv2.putText(
                frame,
                f"Move Right ({left_object[0]} > {frame_width / 2})",
                (0, frame_height - 50),
                cv2.FONT_HERSHEY_SIMPLEX,
                1,
                (0, 0, 255),
